...
import { deleteCmsPage, fetchCmsPages } from "@/services/marketing-api";
...
export function CmsPagesManager() {
  ...
  const [rows, setRows] = useState<CmsPageItem[]>([]);
  ...
  const [sortBy, setSortBy] = useState("pageCode-asc");
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      royalToast.error(getApiErrorMessage(err, "Failed to load CMS pages"));
    } finally {
      setLoading(false);
    }
  ...
  const columns = useMemo(
    (): AdminTableColumn<CmsPageItem>[] => [
      { key: "pageCode", label: "Code" },
      {
        key: "title",
        label: "Title",
        render: (r) => (
      ...
      { key: "slug", label: "Slug" },
      {
        ...
        render: (r) => (
          <span
            className={`admin-status-badge ${r.isPublished ? "active" : "inactive"}`}
          >
  ...
  async function handleDelete(row: CmsPageItem) {
    if (!window.confirm(`Delete CMS page "${row.title}"?`)) return;
    try {
      await deleteCmsPage(row.id);
      royalToast.success("CMS page deleted");
      await loadData();
    } catch (err) {
  ...
  return (
    <>
      <div className="admin-data-tabs">
        <h3 className="admin-data-tabs-title">CMS Pages</h3>
        <Link
          href={NEW_PATH}
        ...
        {viewMode === "table" ? (
          ...
          <AdminDataGrid
            ...
            status={(r) => ({
              ...
              tone: r.isPublished ? "active" : "inactive",
            })}
            fields={(r) => [
              { label: "Code", value: r.pageCode },
