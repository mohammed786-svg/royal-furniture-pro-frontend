...
import { fetchReturns } from "@/services/orders-api";
...
export function ReturnsManager() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      royalToast.error(getApiErrorMessage(err, "Failed to load returns"));
    } finally {
      setLoading(false);
    }
  ...
  const columns = useMemo(
    (): AdminTableColumn<OrderListItem>[] => [
      ...
      { key: "customerName", label: "Customer" },
      {
        ...
        label: "Return Status",
        render: (r) => (
          <span className="admin-status-badge active">{r.currentStatus}</span>
        ),
      },
      {
        key: "totalAmount",
  ...
  return (
    <>
      <div className="admin-data-tabs">
        <h3 className="admin-data-tabs-title">Returns & Refunds</h3>
        <Link
          href={NEW_PATH}
